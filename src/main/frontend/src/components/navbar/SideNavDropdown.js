...
const SideNavDropdown = ({active, bg, color, icon, label, children}) => {
  ...
  return <Box bg={isOpen ? bg : 'inherit'} borderRadius={'8px'} w={'full'}>
    <Button boxSize='initial' justifyContent='space-between' alignItems='center'
            ...
            rightIcon={isOpen ? <FaAngleUp color={color}/> : <FaAngleDown/>}>
      <Flex>
    ...
    <Box overflow={'hidden'} maxHeight={isOpen ? '300px' : '0'} opacity={isOpen ? '1' : 0}
         _last={{borderBottomRadius: '7px'}}
         transition={'max-height .3s cubic-bezier(0.4, 0, 1, 1), opacity .1s cubic-bezier(0.4, 0, 1, 1)'}>
      {children}
    </Box>
  </Box>;
}
